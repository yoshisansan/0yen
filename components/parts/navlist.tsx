...
const NavList: VFC<{onClose: () => void, isHamburger: boolean}> = ({onClose, isHamburger}) => {
  ...
  const parentData = microCMSdata.filter((onePost: OnePostData) => onePost.isParent === true).sort((a: OnePostData, b: OnePostData) => a.time > b.time ? 1 : -1);
  const accordData = parentData.map((parentData: ParentPostData) => {
    ...
    // 記事の投稿日時の昇順データとして格納
    parentData["childData"] = childData.sort((a: OnePostData, b: OnePostData) => a.time > b.time ? 1 : -1);
